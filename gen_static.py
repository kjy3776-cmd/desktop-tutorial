...
def app_html(app: dict) -> str:
    ...
    url      = f"{SITE_URL}/app/{app['name']}.html"
...
def category_html(cat: str, apps: list) -> str:
    ...
    items = ""
    for a in apps:
        items += f"""<a href="{SITE_URL}/app/{_esc(a['name'])}.html" class="app-item">
...
def update_sitemap(apps: list):
    ...
    # 앱 정적 페이지 (SPA URL도 유지)
    for a in apps:
        lines.append(u(f"{SITE_URL}/app/{a['name']}.html", "0.7"))
        lines.append(u(f"{SITE_URL}/?id={a['id']}", "0.5"))
...
def run(new_only=False):
    ...
    for app in apps:
        path = os.path.join(APP_DIR, f"{app['name']}.html")
