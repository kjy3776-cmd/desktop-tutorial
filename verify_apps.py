...
# 메인
# ─────────────────────────────────────────────────────────────
def is_complete(app: dict) -> bool:
    """앱 데이터가 완전한지 확인 — 완전하면 검수 스킵."""
    return (
        app.get("iosUrl", "").startswith("http") and
        app.get("androidUrl", "").startswith("http") and
        app.get("icon", "").startswith("http") and
        len(app.get("desc", "")) >= 50 and
        app.get("spec_and", {}).get("ver", "-") != "-" and
        app.get("slug", "") and
        not app.get("slug", "").startswith("app-")
    )
...
def run(dry_run=False):
    ...
    for i, app in enumerate(apps, 1):
        ...
        # ★ 데이터 완전한 앱은 스킵 (불필요한 외부 요청 절약)
        if is_complete(app):
            skipped += 1
            log(f"[{i}/{len(apps)}] {name} — ✅ 완전 (스킵)")
            report["results"].append({"name": name, "issues": [], "skipped": True})
            continue
    ...
    report["summary"] = {
        "total": len(apps),
        "skipped": skipped,
        "has_issues": sum(1 for r in report["results"] if r.get("issues")),
