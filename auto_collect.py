...
DAILY_LIMIT  = 10         # 하루 최대 등록 상한 (시간당 1개 × 최대 10시간)
MIN_DELAY    = 15         # 앱 사이 최소 대기(초)
MAX_DELAY    = 40         # 앱 사이 최대 대기(초)
HOURLY_LIMIT = 1          # 1회 실행당 등록 개수
...
def run(dry_run=False):
    ...
    today_count = state.get("today_count", {}).get(today, 0)
    if today_count >= DAILY_LIMIT and not dry_run:
        log(f"ℹ️ 오늘({today}) 이미 {today_count}개 등록됨 — 하루 상한({DAILY_LIMIT}개) 도달")
        return
    ...
    target = queue[:HOURLY_LIMIT]  # 1회 실행당 1개
    ...
    today_counts = state.get("today_count", {})
    today_counts[today] = today_count + len(added)
    state["today_count"] = today_counts
    if not dry_run:
